...
def backup():
    ...
    with cd('/tmp'):
        run('mysqldump --user=%s --password=%s --skip-opt --add-drop-table --default-character-set=utf8 --quick student_manager > %s' % (db_user, db_password, f))
...
def deploy():
    ...
    with settings(warn_only=True):
        sudo('supervisorctl stop student_manager')
        sudo('supervisorctl start student_manager')
...
def restore2local():
    ...
    sqls = [
        ...
        'grant select, insert, update, delete on student_manager.* to \'%s\'@\'localhost\' identified by \'%s\';' % (db_user, db_password)
    ]
    for sql in sqls:
        local(r'mysql -uroot -p%s -e "%s"' % (p, sql))
    with lcd(backup_dir):
        local('tar zxvf %s' % restore_file)
    local(r'mysql -uroot -p%s student_manager < backup/%s' % (p, restore_file[:-7]))
    with lcd(backup_dir):
