...
def post(path):
	def _decorator(func):
		...
		return func
	return _decorator
#通过:[字母和下划线开头的跟有n个字母数字下划线]来识别file
...
class Request(object):
	...
	def cookie(self,name,defalut=None):
...
class Jinja2TemplateEngine(TemplateEngine):
	...
	def __call__(self,path,model):
		return self._env.get_template(path).reader(**model).encode('utf-8')
...
def interceptor(pattern='/'):
	def _decorator(func):
		func.__interceptror__ = _build_pattern_fn(pattern)
		return	func
	return _decorator
	
def _build_interceptor_fn(func,next):
	def _wrapper():
		if func.__interceptror__(ctx.request.path_info):
			return func(next)
		else:
			return next()
	return _wrapper
